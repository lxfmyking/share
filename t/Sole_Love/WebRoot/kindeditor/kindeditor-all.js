...
function _createButton(arg) {
	...
	var name = arg.name || '',
		...
		btn = K('<input name="filemanager_close" class="ke-button-common ke-button" type="button" value="' + name + '" />');
	if (arg.click) {
		btn.click(arg.click);
	}
	span.append(btn);
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				if(data.is_photo){//如果是图片
				    var _span=K('<span class="xl_span" data-url="'+K.formatUrl(result.current_url + data.filename, 'absolute')+'" style="position:absolute;display:none;width:102px;background:#0690d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;">删除</span>');
				    div.append(_span);
				}
			}
			K(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
			    ...
			    //alert(data.filename);
			   // window.location.href="imgDel.action?imgurl="+data.filename;
				var url = 'imgDel.action';
				var params = {
				...
				function cbf(data) { 
					if (data) {
						alert("删除成功");
						setTimeout(document.getElementsByName("filemanager_close")[2].click(),100);
						setTimeout(document.getElementsByName("viewServer")[0].click(),2000);
					}else{
						alert("删除失败");
					}
				}, 'json' 
